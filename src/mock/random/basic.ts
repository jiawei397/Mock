...
export default {
    ...
    boolean: function(min: number, max: number, cur: boolean) {
        if (cur !== undefined) {
            min = typeof min !== 'undefined' && !isNaN(min) ? parseInt(String(min), 10) : 1
            max = typeof max !== 'undefined' && !isNaN(max) ? parseInt(String(max), 10) : 1
    ...
    natural: function(min: number | string, max: number | string) {
        ...
        max = typeof max !== 'undefined' ? parseInt(String(max), 10) : 9007199254740992 // 2^53
        return Math.round(Math.random() * (max - min)) + min
    },
    // 返回一个随机的整数。
    integer: function(min: number, max: number) {
        min = typeof min !== 'undefined' ? parseInt(String(min), 10) : -9007199254740992
        max = typeof max !== 'undefined' ? parseInt(String(max), 10) : 9007199254740992 // 2^53
        return Math.round(Math.random() * (max - min)) + min
    },
